...
void frameShowOption(Frame *frame);
...
char *frameGetInput(Frame *frame);
...
void frameInsert(Frame *frame,char *string, int x, int y){
	...
	if(frame->image.width < x + strLen(string))
		frameResize(frame, x + strLen(string), frame->image.height);
...
void frameInsertOption(Frame *frame, char *optionName){
	...
	int newSize = lenOrigin + lenDestiny + 2;
	int qtdOptions = 0;
...
void frameSetTable(Frame *frame, Table *table){
	...
	if(frame->image.height < 15)
			frameResize(frame, frame->image.width, 15);
...
void frameTableTitles(Frame *frame){
	...
	buffer = (char *)realloc(buffer, newWidth + 1);
...
void frameSetInput(Frame *frame, char *subTitle, int inputMaxLen){
	...
	if(frame->inputMax > strLen(frame->inputBuffer)){
		...
		//if(frame->inputMax + 8 > frame->image.width + 3)
	...
	if(lenSubTitle + 2 > frame->inputMax + 2){
		...
		//if(lenSubTitle + 8 > frame->image.width)
...
void frameRefreshMenu(Frame *frame){
	...
	if(frame->qtdOptions){
		...
		frameSetCursor(frame,-1,0);
	}
	
	frameInsertCenter(frame,"W-Anterior : S-Proximo : Enter-OK", frame->image.height - 3);
}
...
void frameRefreshTable(Frame *frame){
	...
	char *buffer = (char *)malloc(frame->image.width + 1);
	int cursor = 1;
	...
	for(i = 0; i < frame->table->qtdLine; i++){
		cursor = 2;
		fail = 0;
		
		if(line  < frame->image.height - 3){
		
			tableGet(frame->table, 0, i + jumpPage, buffer);
			if(buffer[0] == ':'){
				fail = 1;
			}
			...
			for(j = 0; j < frame->table->qtdCol; j++ ){
				if(!fail){
					...
					frameInsert(frame, buffer, cursor - line + j, line);
				}
	...
	if(pages && prevAvaliable && nextAvaliable)
		frameInsertCenter(frame,"O-Opcoes : W-Anterior : S-Proximo",frame->image.height - 3);
	else if(pages && nextAvaliable && !prevAvaliable)
		frameInsertCenter(frame,"O-Opcoes :S-Proximo",frame->image.height - 3);
	else if(pages)
		frameInsertCenter(frame,"O-Opcoes :S-Anterior",frame->image.height - 3);
