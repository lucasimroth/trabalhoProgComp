...
void frameRefreshTable(Frame *frame){
	...
	for(i = 0; i < frame->image.height - 6; i++){
		...
		if(i + jumpPage < frame->table->qtdLine){
			...
			for(j = 0; j < frame->table->qtdCol; j++ ){
				...
				frameInsert(frame, buffer, cursor - i + 1 + linesIgnored, i + 3 - linesIgnored);
	...
	if(pages && prevAvaliable && nextAvaliable)
		frameInsertCenter(frame,"W-Anterior : S-Proximo",frame->image.height - 3);
	else if(pages && nextAvaliable && !prevAvaliable)
		frameInsertCenter(frame,"S-Proximo",frame->image.height - 3);
	else if(pages)
		frameInsertCenter(frame,"S-Anterior",frame->image.height - 3);
